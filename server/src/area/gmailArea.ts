...
function constructEmailBody(to: string, cc: string, bcc: string, subject: string, body: string, attachmentUrl?: string): string {
    ...
    let emailLines = [
        ...
        `<p style="color: #999999;">&copy; ${new Date().getFullYear()} AREAS. All rights reserved. <a href="http://localhost:8081/privacy-policy" style="color: #000000; text-decoration: none;">Privacy Policy</a></p>`,
        `</div>`,
        `</div>`,
        `</body></html>`,
        `--${boundary}--`,
